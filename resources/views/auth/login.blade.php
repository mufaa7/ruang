    ...
    <div class="w-full max-w-md mx-auto">
        ...
        <div class="mt-8">
            ...
            <p class="mt-4 leading-7 text-[#4F4A44]">
                Lanjutkan dari tempat terakhir. Ruangmu masih sama seperti saat kamu meninggalkannya.
            </p>
        </div>

        <!-- Session Status -->
        <x-auth-session-status class="mt-6 rounded-2xl border border-emerald-500/20 bg-emerald-50 p-4 text-sm text-emerald-700" :status="session('status')" />
        ...
        <form method="POST" action="{{ route('login') }}" class="mt-10 space-y-7">
            ...
            <div x-data="{ show: false }">
                ...
                <div class="relative">
                    <input id="password" :type="show ? 'text' : 'password'" name="password" required autocomplete="current-password" placeholder="Masukkan password" class="h-14 w-full rounded-2xl border border-[#D6D0C4] bg-white/60 px-5 pr-14 text-[#1F1F1D] placeholder:text-[#A39D93] outline-none transition duration-300 focus:border-[#1F1F1D] focus:bg-white focus:ring-0 shadow-sm" />
        ...
        <div class="mt-10 border-t border-[#D6D0C4] pt-6">
            <div class="flex items-center justify-between text-xs tracking-[.25em] uppercase font-semibold">
                <span class="text-[#A39D93]">Build 0.1</span>
