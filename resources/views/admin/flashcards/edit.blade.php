...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <h2 class="text-lg font-bold text-black">Edit Flashcard Set: {{ $flashcardSet->title }}</h2>
    ...
    <form action="{{ route('admin.flashcards.update', $flashcardSet) }}" method="POST" class="space-y-6" x-data="flashcardForm()">
        @csrf
        @method('PUT')
        ...
        <div class="dev-card p-6">
            ...
            @php
                $selectedUsers = old('target_users', $flashcardSet->targets->pluck('id')->toArray());
            @endphp
        ...
        <div class="dev-card p-6">
            <div class="flex items-center justify-between mb-4 border-b border-black pb-2">
                <div>
                    ...
                    <p class="text-xs text-slate-500 mt-1">Buat soal-soal kuis pilihan ganda yang akan muncul setelah flashcard tamat. Kosongkan semua soal jika ingin menghapus kuis.</p>
                </div>
            </div>
            ...
            <div>
                ...
                <div class="border border-black overflow-x-auto relative mt-4">
                    <div class="flex items-center justify-between p-4 bg-[#f8f9fa] border-b border-black min-w-[800px]">
                        <h3 class="text-sm font-bold text-black flex items-center gap-2">Questions (Spreadsheet) <span class="text-[10px] text-slate-500 font-normal ml-2">Tip: Kosongkan baris untuk menghapusnya</span></h3>
...
@php
    $initialCards = $flashcardSet->flashcards->map(function($c) {
        return [
            'id' => $c->id,
            'temp_id' => 'existing_' . $c->id,
            'front' => $c->front,
            'back' => $c->back,
        ];
    })->values()->toJson();

    $hasQuiz = $flashcardSet->quiz_id ? 'true' : 'false';
    $quizTitleVal = $flashcardSet->quiz ? addslashes($flashcardSet->quiz->title) : '';
    
    $initialQuestions = '[]';
    if ($flashcardSet->quiz) {
        $initialQuestions = $flashcardSet->quiz->questions->map(function($q) {
            $opts = $q->options ?: [];
            return [
                'id' => $q->id,
...
<script>
    function flashcardForm() {
        return {
            ...
            selectedSubjectId: '{{ old('subject_id', $flashcardSet->subject_id) }}',
            ...
            parseBulk() {
                ...
                if (parsedCount > 0) {
                    ...
                    this.parseMessage = `Successfully parsed and appended ${parsedCount} cards!`;
                    this.bulkText = '';
