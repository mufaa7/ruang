...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <h2 class="text-lg font-bold text-black">Edit Quiz: {{ $quiz->title }}</h2>
    ...
    <form action="{{ route('admin.quizzes.update', $quiz) }}" method="POST" class="space-y-6" x-data="quizForm()">
        @csrf
        @method('PUT')
        ...
        <div class="dev-card p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-4">
                ...
                <div>
                    ...
                    <input type="text" name="title" value="{{ old('title', $quiz->title) }}" required class="w-full border border-black p-2 text-sm text-black bg-white focus:outline-none focus:ring-0 focus:border-black placeholder:text-slate-400" placeholder="e.g. Midterm Physics">
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6" x-show="selectedSubjectId" x-transition>
                ...
                <div>
                    ...
                    <input type="number" name="time_limit_minutes" value="{{ old('time_limit_minutes', $quiz->time_limit_minutes) }}" min="1" class="w-full border border-black p-2 text-sm text-black bg-white focus:outline-none focus:ring-0 focus:border-black">
                </div>
            </div>
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-2 max-h-40 overflow-y-auto p-2 border border-black bg-[#f8f9fa]">
                    ...
                    <template x-for="user in currentUsers" :key="user.id">
                        <label class="flex items-center gap-2 cursor-pointer bg-white p-2 border border-slate-200 hover:border-black transition-colors">
                            <input type="checkbox" name="target_users[]" :value="user.id" x-model="selectedTargets" class="text-black border-slate-300 focus:ring-black">
        ...
        <div class="dev-card p-0 overflow-x-auto relative">
            <div class="flex items-center justify-between p-4 bg-white border-b border-black min-w-[800px]">
                <h3 class="text-sm font-bold text-black flex items-center gap-2">Questions (Spreadsheet) <span class="text-[10px] text-slate-500 font-normal ml-2">Tip: Kosongkan pertanyaan untuk menghapusnya</span></h3>
            ...
            <table class="w-full text-left text-sm whitespace-nowrap min-w-[1000px]">
                <thead>
                    <tr class="bg-[#f8f9fa] text-xs uppercase tracking-wider">
                        ...
                        <th class="px-2 py-2 font-bold text-black border-b border-black w-10 text-center">Act</th>
                    </tr>
                </thead>
...
@php
    $initialQuestions = $quiz->questions->map(function($q) {
        $map = ['A' => 0, 'B' => 1, 'C' => 2, 'D' => 3];
        $correct = $map[$q->correct_answer] ?? 0;
    ...
    $initialTargets = $quiz->targets->pluck('id')->toJson();
@endphp
...
<script>
    function quizForm() {
        return {
            ...
            selectedSubjectId: '{{ old('subject_id', $quiz->subject_id) }}',
            selectedMaterialId: '{{ old('material_id', $quiz->material_id) }}',
            selectedTargets: {!! old('target_users') ? json_encode(old('target_users')) : $initialTargets !!},
            ...
            parseBulk() {
                ...
                if (parsedCount > 0) {
                    ...
                    this.parseMessage = `Successfully parsed and appended ${parsedCount} questions!`;
                    this.bulkText = '';
