<x-app-layout>
    ...
    <div class="max-w-4xl mx-auto pb-12" x-data="flashcardApp()">
        <div class="flex flex-col md:flex-row items-start md:items-center justify-between mb-8 gap-4">
            ...
            <div class="bg-indigo-50 dark:bg-indigo-500/10 px-4 py-2 rounded-xl text-indigo-700 dark:text-indigo-400 font-bold font-geist">
        ...
        <div class="relative w-full max-w-2xl mx-auto perspective-1000" style="min-height: 400px;">
            ...
            <div class="w-full bg-slate-200 dark:bg-slate-700 rounded-full h-1.5 mb-8 overflow-hidden">
                <div class="bg-indigo-600 h-1.5 rounded-full transition-all duration-300" :style="'width: ' + (Math.min(currentIndex + 1, totalCards) / totalCards * 100) + '%'"></div>
            </div>
            ...
            <template x-if="totalCards > 0 && currentIndex < totalCards">
                <div class="relative w-full h-80 cursor-pointer group" @click="flipCard">
                    <div class="absolute inset-0 w-full h-full transition-all duration-500 transform-style-3d shadow-xl rounded-[24px]" :class="{'rotate-y-180': isFlipped}">
                        ...
                        <!-- Back (Definisi) -->
                        <div class="absolute inset-0 w-full h-full backface-hidden bg-indigo-600 dark:bg-indigo-500 rounded-[24px] rotate-y-180 flex flex-col items-center justify-center p-8 text-center text-white overflow-y-auto"
                            :class="{'invisible': !isFlipped && !isAnimating}">
                            <span class="absolute top-6 left-6 text-xs font-bold text-indigo-200 uppercase tracking-widest">Definisi</span>
            ...
            <!-- End of Cards State -->
            <template x-if="totalCards > 0 && currentIndex === totalCards">
                <div class="w-full h-80 bg-white dark:bg-slate-900 rounded-[24px] border-2 border-indigo-100 dark:border-indigo-500/20 flex flex-col items-center justify-center p-8 text-center shadow-sm">
                    <div class="w-16 h-16 rounded-2xl bg-indigo-50 dark:bg-indigo-500/10 flex items-center justify-center text-indigo-600 dark:text-indigo-400 mb-6">
                        <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                    </div>
                    <h2 class="text-2xl font-bold text-slate-800 dark:text-white font-geist mb-2">Tamat Riwayat Kartunya! 🚩</h2>
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <a href="{{ route('latihan.quiz.show', $flashcardSet->quiz_id) }}" class="px-6 py-3 rounded-xl font-bold text-white bg-indigo-600 hover:bg-indigo-700 transition-colors shadow-lg shadow-indigo-600/20 flex items-center gap-2">
