    ...
    <div class="px-4 sm:px-8 md:px-20 py-8 md:py-16 mt-8 sm:mt-0">
        ...
        <div class="text-center mb-12 flex flex-col items-center group/title">
            <div class="relative w-full flex justify-center">
                <input type="text" 
                    value="{{ $bab->bab_label }}" 
                    class="font-bold uppercase text-lg sm:text-[14pt] text-center w-full max-w-xs bg-transparent border border-transparent hover:border-stone-200 hover:bg-stone-50 focus:border-indigo-500 focus:bg-white transition-all rounded px-2 outline-none auto-save-chapter-title"
                    data-url="{{ route('makalah.chapters.update', [$makalah, $bab]) }}"
                    data-field="bab_label">
                <span class="absolute right-0 sm:right-1/4 top-2 text-stone-300 opacity-100 lg:opacity-0 lg:group-hover/title:opacity-100 transition pointer-events-none">✏️</span>
            </div>
            <div class="relative w-full flex justify-center mt-2">
                <input type="text" 
                    value="{{ $bab->title }}" 
                    class="font-bold uppercase text-lg sm:text-[14pt] text-center w-full max-w-md bg-transparent border border-transparent hover:border-stone-200 hover:bg-stone-50 focus:border-indigo-500 focus:bg-white transition-all rounded px-2 outline-none auto-save-chapter-title"
                    data-url="{{ route('makalah.chapters.update', [$makalah, $bab]) }}"
                    data-field="title">
                <span class="absolute right-0 sm:right-1/4 top-2 text-stone-300 opacity-100 lg:opacity-0 lg:group-hover/title:opacity-100 transition pointer-events-none">✏️</span>
            </div>
                ...
                <div class="relative group/sub" data-subchapter-id="{{ $sub->id }}">
                    ...
                    <div class="relative flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2 mb-3 sm:mb-5 group/subtitle">
                        ...
                        <div class="relative flex-1">
                            <input type="text" 
                                value="{{ $sub->title }}" 
                                class="font-bold text-lg sm:text-[12pt] w-full bg-transparent border border-transparent hover:border-stone-200 hover:bg-stone-50 focus:border-indigo-500 focus:bg-white transition-all rounded sm:px-2 outline-none auto-save-chapter-title"
                                data-url="{{ route('makalah.subchapters.update', [$makalah, $sub]) }}"
                                data-field="title">
                            <span class="absolute right-2 top-1 sm:top-1.5 text-stone-300 opacity-100 lg:opacity-0 lg:group-hover/subtitle:opacity-100 transition pointer-events-none">✏️</span>
                        </div>
