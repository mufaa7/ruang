    ...
    <div x-data="{ 
        ...
        <div class="dashboard-card p-6 border-stone-200 dark:border-slate-700/50">
            <div class="flex items-start gap-4">
                <a href="{{ Route::has('subjects.index') ? route('subjects.index') : '#' }}" class="mt-1 w-10 h-10 rounded-xl bg-stone-50 flex items-center justify-center text-neutral-400 hover:text-stone-600 hover:bg-stone-100 border border-stone-200 hover:border-transparent transition-colors dark:bg-slate-900/50 dark:border-slate-700/50">
            ...
            <div class="flex items-center gap-6 mt-8 border-b border-stone-100 overflow-x-auto whitespace-nowrap hide-scrollbar scroll-smooth">
                <button @click="activeTab = 'catatan'" :class="{ 'border-neutral-900 text-neutral-900': activeTab === 'catatan', 'border-transparent text-neutral-400 hover:text-stone-700': activeTab !== 'catatan' }" class="pb-3 min-h-11 text-sm font-bold uppercase tracking-wider border-b-2 transition-colors flex items-center gap-2">Catatan</button>
                <button @click="activeTab = 'materi'" :class="{ 'border-neutral-900 text-neutral-900': activeTab === 'materi', 'border-transparent text-neutral-400 hover:text-stone-700': activeTab !== 'materi' }" class="pb-3 min-h-11 text-sm font-bold uppercase tracking-wider border-b-2 transition-colors flex items-center gap-2">Materi Kelas</button>
                <button @click="activeTab = 'kuis'" :class="{ 'border-neutral-900 text-neutral-900': activeTab === 'kuis', 'border-transparent text-neutral-400 hover:text-stone-700': activeTab !== 'kuis' }" class="pb-3 min-h-11 text-sm font-bold uppercase tracking-wider border-b-2 transition-colors flex items-center gap-2">Latihan Kuis</button>
            </div>
        </div>
        ...
        <div x-show="activeTab === 'catatan'" style="display: none;" x-transition.opacity class="space-y-4">
            <div class="flex flex-col sm:flex-row justify-end gap-3 mb-4">
                ...
                <button @click="showCreateNoteModal = true" class="w-full sm:w-auto justify-center min-h-11 px-4 py-2 bg-neutral-900 border border-neutral-900 text-white hover:bg-stone-700 font-semibold text-xs rounded-xl shadow-sm transition-all active:scale-95 flex items-center gap-2">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div @click="activeNote = {{ json_encode($note) }}; showNoteModal = true" 
                    ...
                    <div class="flex justify-between items-start mb-2">
                        <h4 class="text-base font-bold text-neutral-900 group-hover:text-stone-600 transition-colors line-clamp-1 dark:text-white">{{ $note->title }}</h4>
                        @if($note->is_ai)
        ...
        <div x-show="activeTab === 'materi'" x-transition.opacity class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            ...
            <div class="dashboard-card p-5 flex flex-col h-full group">
                ...
                <div class="flex items-start justify-between gap-3 mb-3">
                    ...
                    <button @click="activeMaterial = {{ json_encode($mat) }}; showMaterialModal = true" class="p-2 text-neutral-400 hover:text-stone-600 hover:bg-stone-100 rounded-lg transition-colors" title="Pratinjau Materi">
                        <i class="ph ph-eye text-xl"></i>
                    </button>
                </div>

                {{-- Judul Materi --}}
                <h4 class="text-base font-bold text-neutral-900 group-hover:text-stone-600 transition-colors line-clamp-1 mb-2 dark:text-white">{{ $mat->title }}</h4>
            ...
            <button @click="showUploadModal = true" class="border-2 border-dashed border-stone-300 rounded-[24px] p-5 flex flex-col items-center justify-center text-neutral-400 hover:text-stone-600 hover:border-neutral-400 hover:bg-stone-50 transition-all group min-h-[220px] dark:border-slate-700">
                <div class="w-12 h-12 bg-white border border-stone-200 rounded-full flex items-center justify-center mb-3 group-hover:border-neutral-400 group-hover:scale-110 transition-all shadow-sm dark:bg-slate-900 dark:border-slate-700/50">
                    <i class="ph ph-upload-simple text-xl text-neutral-500 group-hover:text-stone-600 dark:text-slate-400"></i>
                </div>
                <span class="font-bold text-sm text-neutral-600 group-hover:text-stone-600 dark:text-slate-300">Upload Materi Baru</span>
        ...
        <div x-show="activeTab === 'kuis'" style="display: none;" x-transition.opacity>
            <div class="flex justify-between items-center mb-6">
                <h3 class="text-lg font-bold text-neutral-900 dark:text-slate-100">Daftar Latihan & Kuis</h3>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
                @forelse($quizzes as $quiz)
                <a href="{{ route('latihan.quiz.show', $quiz->id) }}"
                   class="dashboard-card p-5 flex items-center justify-between hover:border-stone-400 hover:shadow-md transition-all cursor-pointer group bg-white dark:bg-slate-900 border border-stone-200 dark:border-slate-800">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <h4 class="text-[15px] font-bold text-neutral-900 group-hover:text-stone-600 transition-colors dark:text-white">{{ $quiz->title }}</h4>
                    ...
                    <div class="shrink-0 text-right">
                        @if($quiz->status === 'selesai')
                            ...
                            <span class="text-[10px] font-bold text-neutral-900 group-hover:underline mt-1.5 block">Review Hasil &rarr;</span>
                        @else
                            <span class="inline-flex items-center justify-center px-4 py-2.5 bg-stone-100 text-neutral-700 font-bold text-xs rounded-xl group-hover:bg-stone-700 group-hover:text-white group-hover:shadow-lg group-hover:shadow-neutral-800/30 transition-all dark:bg-slate-800 dark:text-slate-200">
                                Mulai Kerjakan &rarr;
                            </span>
                        @endif
                    </div>
            ...
            <h3 class="text-lg font-bold text-neutral-900 mb-4 dark:text-slate-100">Flashcards</h3>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
                ...
                <a href="{{ $set->status === 'selesai' ? route('latihan.quiz.show', ['quiz' => $set->quiz_id, 'from' => 'subject']) : route('latihan.flashcard.show', $set->id) }}" 
                   class="dashboard-card p-5 flex items-center justify-between hover:border-orange-300 hover:shadow-md transition-all cursor-pointer group bg-white dark:bg-slate-900 border border-stone-200 dark:border-slate-800">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <h4 class="text-[15px] font-bold text-neutral-900 group-hover:text-stone-600 transition-colors dark:text-white">{{ $set->title }}</h4>
                    ...
                    <div class="shrink-0 text-right">
                        ...
                        @else
                            <span class="inline-flex items-center justify-center px-4 py-2.5 bg-stone-100 text-neutral-700 font-bold text-xs rounded-xl group-hover:bg-stone-700 group-hover:text-white group-hover:shadow-lg group-hover:shadow-orange-500/30 transition-all dark:bg-slate-800 dark:text-slate-200">
                                Mulai Hafalkan &rarr;
                            </span>
                        @endif
                    </div>
        ...
        <div x-show="showMaterialModal" style="display: none;" class="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
            ...
            <div x-show="showMaterialModal" 
                ...
                <div class="p-4 sm:p-5 border-t border-stone-100 shrink-0 flex flex-col-reverse sm:flex-row justify-between sm:items-center gap-3 bg-stone-50/50 rounded-b-[24px]">
                    ...
                    <button class="w-full sm:w-auto justify-center px-5 py-2.5 min-h-11 bg-neutral-900 text-white font-semibold text-sm rounded-xl hover:bg-stone-700 shadow-sm transition-colors active:scale-95">
                        Edit Teks Materi
                    </button>
                </div>
            </div>
        ...
        <div x-show="showNoteModal" style="display: none;" class="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
            ...
            <form x-show="showNoteModal" 
                ...
                <div class="flex items-start justify-between p-6 border-b border-stone-100 shrink-0">
                    <div class="flex-1 mr-4">
                        <div class="flex items-center gap-3 mb-2">
                            <input type="text" name="title" x-model="activeNote.title" class="w-full text-xl font-bold text-neutral-900 font-geist bg-transparent border-0 border-b-2 border-transparent hover:border-stone-200 focus:border-stone-800 focus:ring-0 px-0 py-1 transition-colors dark:text-white" placeholder="Judul Catatan">
                ...
                <div class="p-6 md:p-8 overflow-y-auto flex-1 bg-white dark:bg-slate-900">
                    <textarea name="content" x-model="activeNote.content" class="w-full h-full min-h-[300px] font-serif-editor text-neutral-900 text-lg leading-relaxed bg-transparent border-0 focus:ring-0 px-0 resize-none dark:text-slate-100" placeholder="Isi catatan..."></textarea>
                </div>
                ...
                <div class="p-4 sm:p-6 border-t border-stone-100 shrink-0 bg-stone-50/50 rounded-b-[24px] flex flex-col sm:flex-row justify-between sm:items-center gap-4">
                    ...
                    <div class="flex flex-col-reverse sm:flex-row items-center gap-2 sm:gap-3 w-full sm:w-auto">
                        ...
                        <button type="submit" class="w-full sm:w-auto justify-center px-5 py-2 min-h-11 bg-neutral-900 text-white font-semibold text-sm rounded-xl hover:bg-stone-700 shadow-sm transition-colors flex items-center gap-2 active:scale-95">
        ...
        <div x-show="showUploadModal" style="display: none;" class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div x-show="showUploadModal" 
                ...
                <div class="flex items-center justify-between pb-4 border-b border-stone-100 mb-5">
                    ...
                    <button @click="showUploadModal = false" class="p-1.5 hover:bg-stone-100 rounded-full text-neutral-400 hover:text-stone-600 transition-colors">
                        <i class="ph ph-x text-xl"></i>
                    </button>
                </div>
                ...
                <form action="{{ route('subjects.materials.store', $subject) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
                    ...
                    <div x-data="{ inputType: 'file', fileName: '' }">
                        <div class="flex items-center gap-4 mb-3">
                            <label class="flex items-center gap-2 text-sm font-semibold cursor-pointer text-neutral-700 dark:text-slate-200">
                                <input type="radio" name="input_type" value="file" x-model="inputType" class="text-neutral-900 focus:ring-stone-800 dark:text-white">
                                Upload File
                            </label>
                            <label class="flex items-center gap-2 text-sm font-semibold cursor-pointer text-neutral-700 dark:text-slate-200">
                                <input type="radio" name="input_type" value="text" x-model="inputType" class="text-neutral-900 focus:ring-stone-800 dark:text-white">
                                Paste Text
                            </label>
                        </div>
                        ...
                        <div x-show="inputType === 'file'" onclick="document.getElementById('file-upload').click()" class="relative flex justify-center px-6 pt-5 pb-6 border-2 border-stone-300 border-dashed rounded-xl hover:bg-stone-50 hover:border-neutral-400 transition-colors group cursor-pointer dark:border-slate-700">
                            <div class="space-y-1 text-center pointer-events-none">
                                <i class="ph ph-upload-simple text-4xl text-neutral-400 group-hover:text-stone-600 transition-colors"></i>
                                <div class="flex text-sm text-neutral-600 justify-center dark:text-slate-300">
                                    <label class="relative cursor-pointer rounded-md font-semibold text-neutral-900 hover:text-stone-700 dark:text-white">
                    ...
                    <div class="pt-4 flex flex-col-reverse sm:flex-row items-center sm:justify-end gap-3 border-t border-stone-100 mt-2">
                        ...
                        <button type="submit" class="w-full sm:w-auto justify-center px-5 py-2.5 min-h-11 bg-neutral-900 text-white font-semibold text-sm rounded-xl hover:bg-stone-700 shadow-sm transition-colors flex items-center gap-2 active:scale-95">Simpan Materi</button>
                    </div>
                </form>
            </div>
        </div>
        ...
        <div x-show="showCreateNoteModal" style="display: none;" class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div x-show="showCreateNoteModal" 
                ...
                <div class="flex items-center justify-between pb-4 border-b border-stone-100 mb-5">
                    ...
                    <button @click="showCreateNoteModal = false" class="p-1.5 hover:bg-stone-100 rounded-full text-neutral-400 hover:text-stone-600 transition-colors">
                        <i class="ph ph-x text-xl"></i>
                    </button>
                </div>
                ...
                <form action="{{ route('coretan.store') }}" method="POST" class="space-y-5">
                    ...
                    <div class="pt-4 flex flex-col-reverse sm:flex-row items-center sm:justify-end gap-3 border-t border-stone-100 mt-2">
                        ...
                        <button type="submit" class="w-full sm:w-auto justify-center px-5 py-2.5 min-h-11 bg-neutral-900 text-white font-semibold text-sm rounded-xl hover:bg-stone-700 shadow-sm transition-colors flex items-center gap-2 active:scale-95">Simpan Catatan</button>
                    </div>
                </form>
            </div>
        </div>
        ...
        <div x-show="showAiModal" style="display: none;" class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div x-show="showAiModal" 
                ...
                <div class="flex items-center justify-between pb-4 border-b border-stone-100 mb-5">
                    ...
                    <button @click="showAiModal = false" class="p-1.5 hover:bg-stone-100 rounded-full text-neutral-400 hover:text-stone-600 transition-colors">
                        <i class="ph ph-x text-xl"></i>
                    </button>
                </div>
                ...
                <form action="{{ route('subjects.notes.ai', $course->id) }}" method="POST" class="space-y-5" x-data="{ aiSource: 'materi' }">
                    ...
                    <div>
                        <div class="flex items-center gap-4 mb-4">
                            <label class="flex items-center gap-2 text-sm font-semibold cursor-pointer text-neutral-700 dark:text-slate-200">
                                <input type="radio" name="ai_source" value="materi" x-model="aiSource" class="text-neutral-900 focus:ring-stone-800 dark:text-white">
                                Dari Materi Kelas
                            </label>
                            <label class="flex items-center gap-2 text-sm font-semibold cursor-pointer text-neutral-700 dark:text-slate-200">
                                <input type="radio" name="ai_source" value="manual" x-model="aiSource" class="text-neutral-900 focus:ring-stone-800 dark:text-white">
                                Teks Manual
                            </label>
                        </div>
                    ...
                    <div class="pt-4 flex flex-col-reverse sm:flex-row items-center sm:justify-end gap-3 border-t border-stone-100 mt-2">
                        ...
                        <button type="submit" class="w-full sm:w-auto justify-center px-5 py-2.5 min-h-11 bg-neutral-900 text-white font-semibold text-sm rounded-xl hover:bg-stone-700 shadow-sm transition-colors flex items-center gap-2 active:scale-95">Generate AI Summary</button>
                    </div>
                </form>
            </div>
        </div>
